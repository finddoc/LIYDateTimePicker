...
@property(nonatomic, strong)
    NSLayoutConstraint *dayColumnHeaderHeightConstraint;
...
- (id)initWithFrame:(CGRect)frame {
  ...
  if (self) {
    ...
    self.allDayView.backgroundColor =
        [[UIColor colorWithHexString:@"35b1f1"] colorWithAlphaComponent:0.2];
...
- (void)setDefaultFontFamilyName:(NSString *)defaultFontFamilyName {
  ...
  self.allDayLabel.font =
      [UIFont fontWithName:defaultFontFamilyName size:10.0f];
  self.allDayEventsLabel.font =
      [UIFont fontWithName:defaultFontFamilyName size:10.0f];
}
...
- (void)formatTitle {
  if (self.date) {
    ...
    timeFormatter.dateFormat =
        (self.timeFormat.length == 0) ? @"h:mm a" : self.timeFormat;
    dateFormatter.dateFormat =
        (self.dateFormat.length == 0) ? @"EEEE, MMM d" : self.dateFormat;
    ...
    if (!self.dayTitlePrefix) {
      ...
      dateBase =
          [NSString stringWithFormat:@"%@ %@", self.dayTitlePrefix,
                                     [dateFormatter stringFromDate:self.date]];
    }

    NSMutableAttributedString *dateString =
        [[NSMutableAttributedString alloc] initWithString:dateBase];

    if (self.showTimeInHeader) {
      NSMutableAttributedString *time = [[NSMutableAttributedString alloc]
          initWithString:[timeFormatter stringFromDate:self.date]];
      [time addAttribute:NSFontAttributeName
                   value:[UIFont fontWithName:self.defaultBoldFontFamilyName
                                         size:14.0f]
                   range:NSMakeRange(0, time.length)];

      [dateString appendAttributedString:[[NSAttributedString alloc]
                                             initWithString:@", "]];
      ...
      [dateString addAttribute:NSForegroundColorAttributeName
                         value:self.timeHighlightColor
                         range:NSMakeRange(dateLength, time.length)];
    }
...
- (void)addBackgroundOutline {
  if (!self.backgroundOutline) {
    ...
    self.backgroundOutline.layer.borderColor =
        [UIColor colorWithHexString:@"#d0d0d0"].CGColor;
...
- (void)
configureForDateHeaderWithDayTitlePrefix:(NSString *)dayTitlePrefix
                   defaultFontFamilyName:(NSString *)defaultFontFamilyName
               defaultBoldFontFamilyName:(NSString *)defaultBoldFontFamilyName
                      timeHighlightColor:(UIColor *)timeHighlightColor
                                    date:(NSDate *)date
...
- (void)updateAllDaySectionWithEvents:(NSArray *)allDayEvents {
  ...
  if (allDayEvents.count == 0) {
    ...
    NSArray *allDayEventTitles =
        [allDayEvents map:^id(EKEvent *event) { // TODO: compute once
          return event.title;
        }];
    self.allDayEventsLabel.text =
        [allDayEventTitles componentsJoinedByString:@", "];
  }
...
- (CGFloat)height {
  ...
  if (self.showDateTime) {
    height = self.allDayEvents.count == 0
                 ? kLIYDefaultHeaderHeight
                 : kLIYDefaultHeaderHeight + kLIYAllDayHeight;
  } else {
...
- (void)positionInView:(UIView *)view {
  ...
  self.dayColumnHeaderHeightConstraint =
      [self autoSetDimension:ALDimensionHeight toSize:[self height]];
