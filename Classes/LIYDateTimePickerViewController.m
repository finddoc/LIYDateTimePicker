...
NSString *const MSDayColumnHeaderReuseIdentifier =
    @"MSDayColumnHeaderReuseIdentifier";
NSString *const MSTimeRowHeaderReuseIdentifier =
    @"MSTimeRowHeaderReuseIdentifier";
const CGFloat kLIYGapToMidnight = 20.0f; // TODO should compute, this is from
                                         // the start of the grid to the 12am
                                         // line
...
// TODO submit pull request to MSCollectionViewCalendarLayout so we don't need
// to subclass
...
- (CGFloat)zIndexForElementKind:(NSString *)elementKind
  ...
  if (elementKind == MSCollectionElementKindCurrentTimeHorizontalGridline) {
    CGFloat MSCollectionMinCellZ =
        100.0f; // from MSCollectionViewCalendarLayout.m
...
// reverts the changes that superclass makes for iPad which allows horizontal
// scrolling
...
@interface LIYDateTimePickerViewController () <
    MSCollectionViewDelegateCalendarLayout, UICollectionViewDataSource,
    UICollectionViewDelegate, UIScrollViewDelegate, JTCalendarDataSource>
...
@property(nonatomic, strong)
    NSLayoutConstraint *relativeTimePickerHeightConstraint;
...
@property(nonatomic, strong)
    NSLayoutConstraint *dayPickerContentViewHeightConstraint;
@property(nonatomic, strong)
    NSLayoutConstraint *dayPickerContentViewContainerHeightConstraint;
...
+ (instancetype)timePickerForDate:(NSDate *)date
                         delegate:(id<LIYDateTimePickerDelegate>)delegate {
...
- (void)didRotateFromInterfaceOrientation:
  ...
  [self scrollToTime:self.dateBeforeRotation]; // required because rotation
                                               // causes a scroll away from
                                               // selectedDate
}
...
- (void)switchToMonthPicker {
  ...
  self.dayPickerContentViewHeightConstraint.constant =
      self.dayPickerMonthHeight;
...
- (void)calendarPickerButtonTapped {
  ...
  LIYCalendarPickerViewController *calendarPickerViewController = [LIYCalendarPickerViewController
      calendarPickerWithCalendarsFromUserDefaultsWithEventStore:self.eventStore
                                                     completion:^(
                                                         NSArray *
                                                             newSelectedCalendarIdentifiers) {
                                                       weakSelf
                                                           .visibleCalendars = [newSelectedCalendarIdentifiers
                                                           map:^id(
                                                               NSString *
                                                                   calendarIdentifier) {
                                                             return [weakSelf
  ...
  UINavigationController *navigationController = [[UINavigationController alloc]
      initWithRootViewController:calendarPickerViewController];
...
- (void)commonInit {
  ...
  _showDayPicker = YES;
  self.selectedDate =
      [NSDate date]; // we use the property here to ensure we get a valid date
...
- (void)setupCollectionView {
  self.collectionViewCalendarLayout =
      [[LIYCollectionViewCalendarLayout alloc] init];
  ...
  [self.collectionView registerClass:MSEventCell.class
          forCellWithReuseIdentifier:MSEventCellReuseIdentifier];
  [self.collectionView registerClass:MSTimeRowHeader.class
          forSupplementaryViewOfKind:MSCollectionElementKindTimeRowHeader
                 withReuseIdentifier:MSTimeRowHeaderReuseIdentifier];

  // TODO: we really don't need this, but we're required to register a class for
  // the day column header. Maybe fork MSCollectionViewCalendarLayout to make
  // day column
  // header optional
  [self.collectionView registerClass:MSTimeRowHeader.class
          forSupplementaryViewOfKind:MSCollectionElementKindDayColumnHeader
                 withReuseIdentifier:MSDayColumnHeaderReuseIdentifier];

  // These are optional. If you don't want any of the decoration views, just
  // don't register a class for them.
  [self.collectionViewCalendarLayout
                registerClass:MSCurrentTimeIndicator.class
      forDecorationViewOfKind:MSCollectionElementKindCurrentTimeIndicator];
  [self.collectionViewCalendarLayout
                registerClass:MSCurrentTimeGridline.class
      forDecorationViewOfKind:
          MSCollectionElementKindCurrentTimeHorizontalGridline];
  [self.collectionViewCalendarLayout
                registerClass:MSGridline.class
      forDecorationViewOfKind:MSCollectionElementKindVerticalGridline];
  [self.collectionViewCalendarLayout
                registerClass:MSGridline.class
      forDecorationViewOfKind:MSCollectionElementKindHorizontalGridline];
  [self.collectionViewCalendarLayout
                registerClass:MSTimeRowHeaderBackground.class
      forDecorationViewOfKind:MSCollectionElementKindTimeRowHeaderBackground];
}

- (void)setupCollectionIndexView {
  self.collectionIndexView =
      [BDKCollectionIndexView indexViewWithFrame:CGRectZero indexTitles:@[]];
  [self.collectionIndexView
             addTarget:self
                action:@selector(collectionIndexViewValueChanged:)
...
- (void)setCollectionViewSectionWidth {
  self.collectionViewCalendarLayout.sectionWidth =
...
- (void)setupTimeSelection {
  self.timeDisplayLine = [LIYTimeDisplayLine
           timeDisplayLineInView:self.view
                 withBorderColor:self.defaultColor1
                        fontName:self.defaultSelectedFontFamilyName
                     initialDate:self.selectedDate
      verticallyCenteredWithView:self.collectionView];
...
- (void)relativeTimeButtonTappedWithMinutes:(NSInteger)minutes {
  ...
  // TODO: cannot use dateWithTimeIntervalSinceNow: here because of unit tests.
  // Would be nice for unit tests to not rely on remembering to not use this
  // method.
  NSDate *newDate = [NSDate dateWithTimeInterval:minutes * secondsInMinute
...
- (void)updateDayPickerDate {
  ...
  if (![self.selectedDate isSameDayAsDate:self.dayPicker.currentDateSelected]) {
    ...
    // clear out events immediately because reloadEvents loads events
    // asynchronously
...
- (void)setDayPickerDate:(NSDate *)date {
  // hack documented here:
  // https://github.com/jonathantribouharet/JTCalendar#warning-2
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"kJTCalendarDaySelected"
                    object:date];
...
- (void)setupSaveButton {
  ...
  self.saveButton.titleLabel.font =
      [UIFont fontWithName:self.defaultFontFamilyName size:18.0f];
...
- (void)scrollToTimeInTimePickerMode:(CGFloat)timeFactor {
  ...
  CGFloat timeY =
      (timeFactor * self.collectionViewCalendarLayout.hourHeight) - topInset;
  [self.collectionView setContentOffset:CGPointMake(0, timeY) animated:NO];
}

- (void)scrollToTimeInCalendarMode:(CGFloat)timeFactor {
  CGFloat hourAtMiddleOfCollectionView =
      [self hourAtYCoordinate:self.collectionView.frame.size.height / 2];
  CGFloat timeY = (timeFactor - hourAtMiddleOfCollectionView) *
                  self.collectionViewCalendarLayout.hourHeight;
  timeY = (CGFloat)fmax(0.0f, timeY);
  CGFloat maxYOffset = self.collectionView.contentSize.height -
...
- (NSDate *)nearestValidDateFromDate:(NSDate *)date {
  ...
  NSTimeInterval seconds =
      ceil([date timeIntervalSinceReferenceDate] / scrollIntervalSeconds) *
      scrollIntervalSeconds;
  NSDate *roundDateTime =
...
- (void)setupDayPickerConstraints {
  [self.dayPickerContentViewContainer
      autoPinToTopLayoutGuideOfViewController:self
                                    withInset:0];
  [self.dayPickerContentViewContainer autoPinEdgeToSuperviewEdge:ALEdgeLeading];
  [self.dayPickerContentViewContainer
      autoPinEdgeToSuperviewEdge:ALEdgeTrailing];

  [self.dayPickerContentView
      autoPinEdgesToSuperviewEdgesWithInsets:ALEdgeInsetsZero
                               excludingEdge:ALEdgeTop];
  self.dayPickerContentViewHeightConstraint =
      [self.dayPickerContentView autoSetDimension:ALDimensionHeight
                                           toSize:self.dayPickerWeekHeight];
  self.dayPickerContentViewContainerHeightConstraint =
      [self.dayPickerContentViewContainer autoSetDimension:ALDimensionHeight
                                                    toSize:0];
  [self.dayPickerContentView
      enableWeekMonthPanWithMinimumHeight:self.dayPickerWeekHeight
                         andMaximumHeight:self.dayPickerMonthHeight
      byUpdatingContainerHeightConstraint:
          self.dayPickerContentViewContainerHeightConstraint
           andContentViewHeightConstraint:
               self.dayPickerContentViewHeightConstraint];
...
- (void)updateDayPickerHeight {
  if (self.showDayPicker) {
    self.dayPickerContentViewContainerHeightConstraint.constant =
        self.dayPickerContentViewHeightConstraint.constant;
  } else {
...
- (void)setContainerView:(UIView *)containerView
  ...
  [containerView autoSetDimension:ALDimensionHeight
                           toSize:visible ? height : 0];
  if (!visible) {
...
- (void)setupDayColumnHeaderConstraints {
  ...
  CGFloat columnHeaderPaddingToDayPicker =
      self.showDayPicker && !self.allowTimeSelection ? 10 : 0;
  [self.dayColumnHeader autoPinEdge:ALEdgeTop
                             toEdge:ALEdgeBottom
                             ofView:self.dayPickerContentView
                         withOffset:columnHeaderPaddingToDayPicker];
}
...
- (void)updateRelativeTimePickerContainerHeight {
  BOOL relativeTimePickerVisible =
      self.showRelativeTimePicker && self.allowTimeSelection;
  ...
  if (self.relativeTimePickerHeightConstraint) {
    ...
    self.relativeTimePickerHeightConstraint =
        [self.relativeTimePickerContainer autoSetDimension:ALDimensionHeight
                                                    toSize:height];
  }
...
- (void)setupSaveButtonConstraints {
  [self pinView:self.saveButtonContainer
      belowView:self.relativeTimePickerContainer];
  [self.saveButtonContainer autoPinToBottomLayoutGuideOfViewController:self
                                                             withInset:0];
  [self setContainerView:self.saveButtonContainer
                 visible:self.saveButton != nil
              withHeight:LIYSaveButtonHeight];
}
...
- (void)setupCollectionIndexViewConstraints {
  ...
  [self.collectionIndexView autoPinEdge:ALEdgeTop
                                 toEdge:ALEdgeTop
                                 ofView:self.collectionView];
  [self.collectionIndexView autoPinEdge:ALEdgeBottom
                                 toEdge:ALEdgeBottom
                                 ofView:self.collectionView];
}
...
/// y is measured where 0 is the top of the collection view (after day column
/// header and optionally all day event view)
- (NSDate *)dateFromYCoordinate:(CGFloat)y {
  CGFloat intervalsPerHour = 60.0f / self.scrollIntervalMinutes;
  CGFloat hour = (CGFloat)(
      round([self hourAtYCoordinate:y] * intervalsPerHour) / intervalsPerHour);
  NSCalendar *cal = [NSCalendar currentCalendar];
  NSDateComponents *dateComponents =
      [cal components:NSCalendarUnitEra | NSCalendarUnitYear |
                      NSCalendarUnitMonth | NSCalendarUnitDay
             fromDate:self.dayPicker.currentDateSelected];
...
- (void)reloadEvents {
  if (![self isViewLoaded] || !self.visibleCalendars ||
      self.visibleCalendars.count == 0) {
  ...
  typeof(self) __weak weakSelf = self;
  [[LIYCalendarService sharedInstance]
      eventsForDate:self.selectedDate
          calendars:self.visibleCalendars
         completion:^(NSArray *nonAllDayEvents, NSArray *allDayEvents) {
  ...
  [self.eventStore
      requestAccessToEntityType:EKEntityTypeEvent
                     completion:^(BOOL granted, NSError *error) {
                       ...
                       NSArray *calendarIdentifiers = [LIYCalendarPickerViewController
                           selectedCalendarIdentifiersFromUserDefaultsForEventStore:
                               weakSelf.eventStore];
                       dispatch_async(dispatch_get_main_queue(), ^{
                         weakSelf.visibleCalendars = [calendarIdentifiers
                             map:^id(NSString *calendarIdentifier) {
                               return [weakSelf.eventStore
...
- (CGFloat)hourAtYCoordinate:(CGFloat)y {
  CGFloat hour = (y + self.collectionView.contentOffset.y - kLIYGapToMidnight) /
                 self.collectionViewCalendarLayout.hourHeight;
...
- (void)addCalendarPickerButton {
  ...
  self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]
      initWithTitle:@"Calendars"
              style:UIBarButtonItemStylePlain
             target:self
             action:@selector(calendarPickerButtonTapped)];
}

- (void)setupDayColumnHeader {
  self.dayColumnHeader = [MSDayColumnHeader new];
  if (self.showDateInDayColumnHeader) {
    [self.dayColumnHeader
        configureForDateHeaderWithDayTitlePrefix:self.dayTitlePrefix
                           defaultFontFamilyName:self.defaultFontFamilyName
                       defaultBoldFontFamilyName:
                           self.defaultSelectedFontFamilyName
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  if (self.dayPicker.panGestureState == UIGestureRecognizerStatePossible &&
      self.allowTimeSelection && self.viewHasAppeared) {
...
- (void)setAllDayEvents:(NSArray *)allDayEvents {
  ...
  if (self.selectedDate && self.allowTimeSelection) {
    [self scrollToTime:self.selectedDate]; // it's possible the scroll changed
                                           // when all day now shows
  }
}
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  NSPredicate *predicate =
      [NSPredicate predicateWithFormat:@"SELF =[c] %@", event.calendar.title];
  NSArray *matchingCalendars =
      [self.calendarNamesToFilterForEdit filteredArrayUsingPredicate:predicate];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  MSEventCell *cell = [collectionView
      dequeueReusableCellWithReuseIdentifier:MSEventCellReuseIdentifier
                                forIndexPath:indexPath];

  // this is a safety check since we were seeing a crash here. not sure how this
  // would happen.
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
  ...
  if (kind == MSCollectionElementKindDayColumnHeader) {
    ...
    MSTimeRowHeader *timeRowHeader = [collectionView
        dequeueReusableSupplementaryViewOfKind:kind
                           withReuseIdentifier:MSTimeRowHeaderReuseIdentifier
                                  forIndexPath:indexPath];
    timeRowHeader.time = [self.collectionViewCalendarLayout
        dateForTimeRowHeaderAtIndexPath:indexPath];
    view = timeRowHeader;
  }
...
collectionView:(UICollectionView *)collectionView
        layout:(MSCollectionViewCalendarLayout *)collectionViewCalendarLayout
 dayForSection:(NSInteger)section {
  return self.selectedDate;
}

- (NSDate *)collectionView:(UICollectionView *)collectionView
                         layout:(MSCollectionViewCalendarLayout *)
  ...
  startDate = fmax(startDate,
                   [[self.selectedDate beginningOfDay] timeIntervalSince1970]);
...
- (NSDate *)collectionView:(UICollectionView *)collectionView
 ...
 endTimeForItemAtIndexPath:(NSIndexPath *)indexPath {
  ...
  NSDate *startDate = [self collectionView:self.collectionView
  ...
  if (endDate - [startDate timeIntervalSince1970] < 15 * 60) {
    endDate = [startDate timeIntervalSince1970] +
              30 * 60; // set to minimum 30 min gap
  }
  return [NSDate dateWithTimeIntervalSince1970:endDate];
}

- (NSDate *)
currentTimeComponentsForCollectionView:(UICollectionView *)collectionView
                                layout:(MSCollectionViewCalendarLayout *)
                                           collectionViewCalendarLayout {
  return [NSDate date];
}
...
- (BOOL)calendarHaveEvent:(JTCalendar *)calendar date:(NSDate *)date {
  return [[LIYCalendarService sharedInstance] calendars:self.visibleCalendars
